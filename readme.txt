...
CardBox v1.2.1 では、ワークスペースごとの画面分割サイズ保存に加えて、プロンプト/説明/メディア欄などの折りたたみ開閉状態もワークスペース別に保存・復元するようにしました。新規カードのメディア保存先は v1.2.0 以降、assets/YYYYMM/prompt_xxxxxx 形式に分散されます。既存カードの assets は移動せず、従来配置のまま参照します。
